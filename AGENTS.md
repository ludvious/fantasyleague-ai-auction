...
- Coach files are only discovered when `paths.coaches` is set — a `coachAgent_*.md` outside that directory does nothing unless `configs/llm.yaml` (or another config) points there (the example points at `agents/coach/`). The `Bidder` protocol requires `observe(result, squad)`, so every test double must implement it (no-op is fine).
...
- `configs/default.yaml` is the canonical schema (deterministic bidders); `configs/llm.yaml` is the CoachAgent example (its `paths.coaches: "agents/coach"` discovers the example `coachAgent_*.md` files). Unknown YAML sections/fields are ignored; CLI flags override YAML.
