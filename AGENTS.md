...
venv/bin/python main.py benchmark --config configs/default.yaml --runs 2
```
...
- `agents/` — bidders implementing the `Bidder` protocol (`base_agent.py`). `coach_agent.py` = `CoachAgent` (the only bidder); `llm_client.py` = `LlmClient` + tool/search schemas; `coach_loader.py` discovers `coachAgent_*.md` profiles (`paths.coaches`); `coach_prompt.py` renders `prompts/system_prompt.md` with domain rules.
...
- Coach files are only discovered when `paths.coaches` is set — a `coachAgent_*.md` outside that directory does nothing unless `configs/default.yaml` (or another config) points there (the default points at `agents/coach/`). The `Bidder` protocol requires `observe(result, squad)`, so every test double must implement it (no-op is fine).
...
- Pool exhaustion writes a `checkpoint.llm.yaml` sidecar next to the checkpoint; resuming a checkpoint requires it and ignores `--config`. The sidecar stores the resolved `system_prompt` per buyer (schema stays 1), so resume needs neither the YAML nor the `.md` files.
...
- `configs/default.yaml` is the canonical schema and the CoachAgent example (its `paths.coaches: "agents/coach"` discovers the example `coachAgent_*.md` files); every buyer is an LLM bidder, so the global `llm` block is always required. Unknown YAML sections/fields are ignored; CLI flags override YAML.
